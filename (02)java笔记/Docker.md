...
（2）再次启动私服容器(如果私服仓库的容器没有启动，则通过下面的命令去启动)

```
docker start registry
```

（3）上传标记的镜像（上传的话，要确保私服仓库的容器要启动）
...
```
1.在其他的电脑上（测试人员、运维人员）安装docker，并且配置daemon.json，信任私服仓库地址
2.docker pull 192.168.184.141:5000/jdk1.8
```
